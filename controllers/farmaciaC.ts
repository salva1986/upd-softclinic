...
export const verTodos = async (req: any, res: Response) => {
    ...
    if (req.params.estado === 'noactivos') {
        ...
        if (texto === 'undefined') {
            requisiciones.items = await Requisicion.find({ estado: true, unidad: 'farmacia' }).populate('producto', 'nombre stock').skip((req.params.page - 1) * 30).limit(30).sort({ nombres: 1 });
            requisiciones.total_count = await Requisicion.countDocuments({ estado: true, unidad: 'farmacia' });
        } else {
            requisiciones.items = await Requisicion.find({ $and: [{ $or: [{ nombres: new RegExp(texto, 'i') }] }, { estado: true, unidad: 'farmacia' }] }).populate('producto', 'nombre stock').skip(0).limit(30).sort({ nombres: 1 });
...
export const finalizarRequisicion = async (req: any, res: Response) => {
  ...
  let _res
   ...
   const now= new Date();
    const rs = await Requisicion.updateOne({ _id: id, estadopedido: 'Aceptado' }, { $set: { estadopedido: 'Finalizado', fechafinalizacion: now } });

    if (rs.nModified) {
        idproducto = prod[0].producto;
        _cantidad = prod[0].cantidad;
        let restar = await Producto.updateOne({ _id: idproducto }, { $inc: { stock: _cantidad * -1, stockfarmacia: _cantidad } });

        if(restar.nModified){
             await Stock.updateOne({ unidad: 'farmacia' }, { $push: { entradas: {producto: idproducto, cantidad: _cantidad} }, $inc: { stock: _cantidad } });
         }
    }
