...
export const verTodos = async (req: any, res: Response) => {
    ...
    if (req.params.estado === 'noactivos') {
        ...
        if (texto === 'undefined') {
           ...
           requisiciones.items = await Requisicion.find({ estado: true, unidad: 'insumo' }).populate('producto', 'nombre stock').skip((req.params.page - 1) * 30).limit(30).sort({ field: 'asc', fechapedido: 1 });
            requisiciones.total_count = await Requisicion.countDocuments({ estado: true, unidad: 'insumo' });
        } else {
           ...
           requisiciones.items = await Requisicion.find({ $and: [{ $or: [{ nombres: new RegExp(texto, 'i') }] }, { estado: true, unidad: 'insumo' }] }).populate('producto', 'nombre stock').skip(0).limit(30).sort({ field: 'asc', fechapedido: 1 });
...
export const finalizarRequisicion = async (req: any, res: Response) => {
    ...
    if (rs.nModified) {
        ...
        let restar = await Producto.updateOne({ _id: idproducto }, { $inc: { stock: _cantidad * -1, stockinsumo: _cantidad }});
        if(restar.nModified){
            await Stock.updateOne({ unidad: 'insumo' }, { $push: { entradas: {producto: idproducto, cantidad: _cantidad} }, $inc: { stock: _cantidad } });
        }
    }
