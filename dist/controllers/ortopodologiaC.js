...
exports.verTodos = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (req.params.estado === 'noactivos') {
        ...
        if (texto === 'undefined') {
            ...
            requisiciones.items = yield requisicion_model_1.Requisicion.find({ estado: true, unidad: 'ortopodologia' }).populate('producto', 'nombre stock').skip((req.params.page - 1) * 30).limit(30).sort({ field: 'asc', fechapedido: 1 });
            requisiciones.total_count = yield requisicion_model_1.Requisicion.countDocuments({ estado: true, unidad: 'ortopodologia' });
        }
        else {
            ...
            requisiciones.items = yield requisicion_model_1.Requisicion.find({ $and: [{ $or: [{ nombres: new RegExp(texto, 'i') }] }, { estado: true, unidad: 'ortopodologia' }] }).populate('producto', 'nombre stock').skip(0).limit(30).sort({ field: 'asc', fechapedido: 1 });
            requisiciones.total_count = Object.keys(requisiciones.items).length;
        }
    }
...
exports.finalizarRequisicion = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (rs.nModified) {
        ...
        let restar = yield producto_model_1.Producto.updateOne({ _id: idproducto }, { $inc: { stock: _cantidad * -1, stockortopodologia: _cantidad } }); //Descargar de Bodega Central
        ...
        if (restar.nModified) { //Cargar productos a stock de cada unidad
            yield stock_model_1.Stock.updateOne({ unidad: 'ortopodologia' }, { $push: { entradas: { producto: idproducto, cantidad: _cantidad } }, $inc: { stock: _cantidad } });
        }
    }
    res.json({
        rs
