...
exports.verTodos = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (req.params.estado === 'noactivos') {
        /*         if (texto === 'undefined') {
                    requisiciones.items = await Requisicion.find({estado: false, unidad: 'insumo'}).populate('producto', 'nombre, stock').skip((req.params.page - 1) * 30).limit(30).sort({ nombres: 1 });
                    requisiciones.total_count = await Requisicion.countDocuments({estado: false});
                } else {
                  requisiciones.items = await Requisicion.find({$and: [{ $or: [{ nombres: new RegExp(texto, 'i'), hc: new RegExp(texto, 'i') }] }, {estado: false, unidad: 'insumo'}]}).populate('producto', 'nombrestock').skip(0).limit(30).sort({ nombres: 1});
        ...
        if (texto === 'undefined') {
            // requisiciones.items = await Requisicion.find({ estado: true, unidad: 'insumo' }).populate('producto', 'nombre stock').skip((req.params.page - 1) * 30).limit(30).sort({ estadopedido: 1 });
            requisiciones.items = yield requisicion_model_1.Requisicion.find({ estado: true, unidad: 'insumo' }).populate('producto', 'nombre stock').skip((req.params.page - 1) * 30).limit(30).sort({ field: 'asc', fechapedido: 1 });
            requisiciones.total_count = yield requisicion_model_1.Requisicion.countDocuments({ estado: true, unidad: 'insumo' });
        }
        else {
            // requisiciones.items = await Requisicion.find({ $and: [{ $or: [{ nombres: new RegExp(texto, 'i') }] }, { estado: true, unidad: 'insumo' }] }).populate('producto', 'nombre stock').skip(0).limit(30).sort({ estadopedido: 1 });
            requisiciones.items = yield requisicion_model_1.Requisicion.find({ $and: [{ $or: [{ nombres: new RegExp(texto, 'i') }] }, { estado: true, unidad: 'insumo' }] }).populate('producto', 'nombre stock').skip(0).limit(30).sort({ field: 'asc', fechapedido: 1 });
            requisiciones.total_count = Object.keys(requisiciones.items).length;
        }
    }
...
exports.finalizarRequisicion = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const rs = yield requisicion_model_1.Requisicion.updateOne({ _id: id, estadopedido: 'Aceptado' }, { $set: { estadopedido: 'Finalizado', fechafinalizacion: new Date() } });
    if (rs.nModified) {
        idproducto = prod[0].producto;
        _cantidad = prod[0].cantidad;
        let restar = yield producto_model_1.Producto.updateOne({ _id: idproducto }, { $inc: { stock: _cantidad * -1, stockinsumo: _cantidad } });
        if (restar.nModified) {
            yield stock_model_1.Stock.updateOne({ unidad: 'insumo' }, { $push: { entradas: { producto: idproducto, cantidad: _cantidad } }, $inc: { stock: _cantidad } });
        }
    }
    res.json({
        rs
